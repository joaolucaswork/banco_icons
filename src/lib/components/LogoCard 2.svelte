<script lang="ts">
  ...
  // Computed: show checkbox when hovering OR when there are any selections
  const showCheckbox = $derived(isHovered || hasAnySelection);
  ...
  // Wrapper function to handle selection toggle
  function handleToggleSelection() {
    // Call the parent's toggle function directly
    // Let the parent state update flow back through isSelected prop
    onToggleSelection();
  }
...
>
  ...
  >
    ...
    {#if showCheckbox}
      <div
        class="absolute top-3 left-3 z-30 transition-opacity duration-200"
                      ...
                      <TooltipTrigger asChild>
                        {#snippet child({ props })}
                          <button
                            ...
                            style={colorOption.isGradient &&
                            colorOption.secondaryValue
                          ...
                          >
                            ...
                            {#if color === colorOption.value}
                              <div
                                ...
                                style="color: {isDarkColor(colorOption.value)
                                  ? '#ffffff'
                                  : '#000000'}; text-shadow: 0 0 3px {isDarkColor(
                                  colorOption.value,
                                )
                                  ? '#000000'
                                  : '#ffffff'};"
