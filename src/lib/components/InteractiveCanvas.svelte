<script>
  ...
  // Cache for dotted pattern to avoid redrawing
  let dottedPatternCache = new Map();
  let originalDottedPatternCache = new Map();
  ...
  function drawDottedPattern(context, width, height) {
    ...
    // Create cache key based on dimensions and color
    const cacheKey = `${width}x${height}_${dotColor}`;

    // Check if we have a cached pattern
    if (dottedPatternCache.has(cacheKey)) {
      const cachedPattern = dottedPatternCache.get(cacheKey);
      context.globalAlpha = 0.3;
      context.drawImage(cachedPattern, 0, 0);
      context.globalAlpha = 1;
      return;
    }

    // Create pattern on a separate canvas for caching
    ...
    // Cache the pattern
    dottedPatternCache.set(cacheKey, patternCanvas);

    // Draw the pattern with opacity
    context.globalAlpha = 0.3;
  ...
  function drawDottedPatternOriginal(context, width, height) {
    ...
    // Create cache key based on dimensions and color
    const cacheKey = `${width}x${height}_${originalDotColor}`;

    // Check if we have a cached pattern
    if (originalDottedPatternCache.has(cacheKey)) {
      const cachedPattern = originalDottedPatternCache.get(cacheKey);
      context.globalAlpha = 0.3;
      context.drawImage(cachedPattern, 0, 0);
      context.globalAlpha = 1;
      return;
    ...
    patternCtx.fillStyle = originalDotColor;
    ...
    // Cache the pattern
    originalDottedPatternCache.set(cacheKey, patternCanvas);
  ...
  // Only redraw when visual properties change, not size properties
  $effect(() => {
    // Track only the dependencies that should cause visual redraw
    const _ = [
      svgContent,
      previewBackground,
      dotColor,
          ...
          <TooltipTrigger asChild>
            {#snippet child({ props })}
              ...
              >
                <ZoomIn
                  class="h-4 w-4"
                  style="color: {iconColor === 'text-black'
                    ? '#000000'
